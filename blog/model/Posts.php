...
class Posts extends Database{
    ...
    public function postsLimited($from = 0){
        $prepare = $this->prepare("SELECT posts.*, user.firstName, user.middleName, user.lastName FROM posts LEFT JOIN user ON posts.userId = user.id ORDER BY dateCreated DESC LIMIT $from, 2");
        $prepare->execute();
        return $prepare->fetchAll();
    }
